...
def fit_function(x, a, b, c, d, e, f, g):
    return a*ycon + b*(calc.intpol(np.sin((2*math.pi*np.arange(len(sat_time)))/6120.85 + c), day, 0, sat_time, bin_time_mid, detector)[0]) - d*earth_ang_bin - e*sun_ang_bin + f*np.cos(distance + g)# + f*x
