...
def ysin(b, c, counts):
    ysin = b*(calc.intpol(np.sin((2*math.pi*np.arange(len(sat_time)))/6120.85 + c), day, 0, sat_time, bin_time_mid, detector)[0])
    ysin[np.where(counts == 0)] = 0
    return ysin

def orbit(f, g, counts):
    orbit = f*(calc.intpol(np.sin((2*math.pi*np.arange(len(sat_time)))/5531 + g), day, 0, sat_time, bin_time_mid, detector)[0])
    orbit[np.where(counts == 0)] = 0
    return orbit

def fit_function(x, a, b, c, d, e, f, g):
    return a*ycon + ysin(b, c, echan_counts[0]) + d*earth_ang_bin + e*sun_ang_bin + orbit(f, g, echan_counts[0])# + f*x
...
ysin = ysin(b, c, echan_counts[0])
orbit = orbit(f, g, echan_counts[0])

fit_curve = a*ycon + ysin + d*earth_ang_bin + e*sun_ang_bin + orbit# + f*bin_time_mid_norm
...
plot5 = ax3.plot(sat_time_bin, ysin, 'g--', label = 'Geographic orbit')
plot6 = ax3.plot(sat_time_bin, ycon, 'b--', label = 'Constant background')
plot7 = ax3.plot(sat_time_bin, orbit, 'y--', label = 'J2000 orbit')
...
plot5 = ax1.plot(sat_time_bin, ysin, 'g--', label = 'Geographical orbit')
plot6 = ax1.plot(sat_time_bin, a*ycon, 'b--', label = 'Constant background')
plot7 = ax1.plot(sat_time_bin, orbit, 'y--', label = 'J2000 orbit')
