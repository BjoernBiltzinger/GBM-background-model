...
config_default_path = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "config_default.py"
)
config_custom_path = os.path.join(
    get_path_of_external_data_dir(), "fits", "config_custom.py"
)
...
if args.config_file is not None:
    ...
    config_custom_path = os.path.join(
        get_path_of_external_data_dir(), "fits", args.config_file + ".py"
    )
...
saa_calc = SAA_calc(
    data,
    time_after_SAA=time_after_SAA,
    short_time_intervals=short_time_intervals,
    nr_decays=nr_decays,
)
print_progress("Done")
...
ep = ExternalProps(
    date, det=detector, bgo_cr_approximation=setup_dict["bgo_cr_approximation"]
)
print_progress("Done")
...
print_progress(
    "Precalculate responses for {} points on sphere around detector...".format(Ngrid)
)
resp = Response_Precalculation(
    detector, date, echan_list, Ngrid=Ngrid, data_type=data_type
)
print_progress("Done")
...
###################################
print_progress(
    "Precalculate geometry for {} times during the day...".format(n_bins_to_calculate)
)
...
########## Setup options ###########
assert (setup_dict["fix_earth"] and setup_dict["fix_cgb"]) or (
    not setup_dict["fix_earth"] and not setup_dict["fix_cgb"]
...
mn_fit.minimize_multinest(
    n_live_points=num_live_points, const_efficiency_mode=const_efficiency_mode
)
...
for index, echan in enumerate(echan_list):
    ...
    residual_plot = plotter.display_model(
        ...
        show_grb_trigger=show_grb_trigger,
    ...
    if rank == 0:
        residual_plot.savefig(
            output_dir
            + "residual_plot_{}_det_{}_echan_{}_bin_width_{}.pdf".format(
                date, detector, echan, bin_width
            ),
            dpi=300,
        )
