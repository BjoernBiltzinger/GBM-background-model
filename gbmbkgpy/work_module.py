...
class readfile(object):
    ...
    def cspec(self, detector_name, day, seconds = 0):
        ...
        chan_energies = np.zeros((len(emin),2), float) #combine the energy limits of the energy channels in one matrix
    ...
    def fits_data(self, day, detector_name, echan, data_type = 'ctime', sources_number = 'None'):
        ...
        #crab_ang_bin = fit_data.Crab_curve
        #scox_ang_bin = fit_data.Scox_curve
        #cygx_ang_bin = fit_data.Cygx_curve
        ...
        sources_ang_bin = sources_ang_bin.view(sources_ang_bin.dtype[0]).reshape(sources_ang_bin.shape + (-1,)).astype(np.float).T
        ...
        return residuals, counts, fit_curve, cgb, magnetic, earth_ang_bin, sun_ang_bin, sources_fit_curve, plot_time_bin, plot_time_sat, fit_coeffs, sources_ang_bin, sources_names, sources_coeff
    ...
    def point_sources(self):
        ...
        coordinates = np.array([source_ra, source_dec])#merge the arrays
        return source_names, coordinates
...
class calculate(object):
    ...
    def curve_fit_plots(self, day, detector_name, echan, data_type = 'ctime', plot = 'yes', write = 'no', burst_time = 'none'):
        ...
        for i in range(0, sources_number):
            ...
            src_data = calculate.burst_ang_bin(calculate(), detector_name, day, src_ra, src_dec, bin_time_mid, data_type)
            src_ang_bin = src_data[0]
            src_ang_bin = calculate.ang_eff(calculate(), src_ang_bin, echan, data_type)[0]
            src_occ = calculate.src_occultation_bin(calculate(), day, src_ra, src_dec, bin_time_mid, detector_name, data_type)[0]
        ...
        earth_ang_bin[earth_ang_bin != 0] = earth_ang_bin[earth_ang_bin != 0] - np.mean(earth_ang_bin[earth_ang_bin != 0], dtype=np.float64)
        ...
        def wrapper_fit_function(x, deaths, sources_number, addition, a, b, c, d, *args):
            ...
            return fit_function(x, a, b, c, d, addition, sources_coeff, np.fabs(exp1), np.fabs(exp2), deaths, sources_number)
        ...
        if os.access(addition_file, os.F_OK):
            ...
            for addition in range(0, 21):
                ...
                try:
                    fit_results = optimization.curve_fit(lambda x, a, b, c, d, *args: wrapper_fit_function(x, deaths, sources_number, addition, a, b, c, d, *args), bin_time_mid, counts, x0, sigma, maxfev = 10000)
        ...
        fit_results = optimization.curve_fit(lambda x, a, b, c, d, *args: wrapper_fit_function(x, deaths, sources_number, addition, a, b, c, d, *args), bin_time_mid, counts, x0, sigma, maxfev = 15000)
        coeff = fit_results[0]
        #pcov = fit_results[1]
        ...
        if plot == 'yes' or plot == 'yes!':
            ...
            plt.title(str(data_type) + '-fit of the ' + detector_name + '-detector on the ' + ordinal(int(str(day)[4:6])) + ' ' + date.strftime('%B')[0:3] + ' ' + str(year), fontsize=45)
            ...
            plt.title(str(data_type) + '-fit residuals of the ' + detector_name + '-detector on the ' + ordinal(int(str(day)[4:6])) + ' ' + date.strftime('%B')[0:3] + ' ' + str(year), fontsize=45)
        ...
        if os.path.isfile(fitsfilepath_data) == False:
            writefile.fits_data(writefile(), day, detector_name, echan, data_type, residuals, counts, fit_curve, cgb, magnetic, earth_ang_bin, sun_ang_bin, sources_fit_curve, plot_time_bin, plot_time_sat, a, b, c, d, sources_ang_bin, sources_names, sources_coeff, saa_coeffs)
        elif write == 'yes':
            writefile.fits_data(writefile(), day, detector_name, echan, data_type, residuals, counts, fit_curve, cgb, magnetic, earth_ang_bin, sun_ang_bin, sources_fit_curve, plot_time_bin, plot_time_sat, a, b, c, d, sources_ang_bin, sources_names, sources_coeff, saa_coeffs)
    ...
    def curve_fit_analysis(self, day, detector_names, echans, data_type = 'ctime', analysis = 'energychannels', echanstart = 'None', echanstop = 'None', show = 'no', timeframe = 'no', burst_time = 'none', binning = 1):
...
##################################################################################################################
        ...
        if analysis == 'energychannels':
                ...
                if echans.size == 1:
                    if detector_names.size == 1:
                        ...
                        for detector_name in detector_names:
                            ...
                            plot_time_bin = data[8]
                            plot_time_sat = data[9]
                    data_channels = np.array([residuals, counts, fit_curve, cgb, magnetic, earth_ang_bin, sun_ang_bin, sources_fit_curve, plot_time_bin, plot_time_sat])
                    ...
                    for echan in echans:
                        ...
                        temp_data = np.array([residuals, counts, fit_curve, cgb, magnetic, earth_ang_bin, sun_ang_bin, sources_fit_curve, plot_time_bin, plot_time_sat])
            ...
            for j in range(0, length):
                ...
                plt.title('Combined detectors on the ' + ordinal(int(str(day)[4:6])) + ' ' + date.strftime('%B')[0:3] + ' ' + str(year), fontsize=40)
                ...
                figure_name = str(day) + '_combined_' + str(analysis) + '_' + str(data_type) + '_' + detec_name + '_e' + echan_name + '.png'
                ...
                plt.title('Combined detector residuals on the ' + ordinal(int(str(day)[4:6])) + ' ' + date.strftime('%B')[0:3] + ' ' + str(year), fontsize=40)
                ...
                figure_name = str(day) + '_combined_' + str(analysis) + '_' + str(data_type) + '_' + str(detec_name) + '_e' + str(echan_name) + '_res.png'
...
##################################################################################################################
                ...
                if detector_names.size == 1:
                    if echans.size == 1:
                        ...
                        for echan in echans:
                            ...
                            plot_time_bin = data[8]
                            plot_time_sat = data[9]
                    data_detectors = np.array([residuals, counts, fit_curve, cgb, magnetic, earth_ang_bin, sun_ang_bin, sources_fit_curve, plot_time_bin, plot_time_sat])
            ...
            for j in range(0, detector_names.size):
                ...
                plt.title('Combined energy channels on the ' + ordinal(int(str(day)[4:6])) + ' ' + date.strftime('%B')[0:3] + ' ' + str(year), fontsize=40)
                ...
                plt.title('Combined energy channel residuals on the ' + ordinal(int(str(day)[4:6])) + ' ' + date.strftime('%B')[0:3] + ' ' + str(year), fontsize=40)
                ...
                figure_name = str(day) + '_combined_' + str(analysis) + '_' + str(data_type) + '_' + detec_name + '_e' + echan_name + '_res.png'
...
##################################################################################################################
                ...
                if detector_names.size == 1:
                    if echans.size == 1:
                        ...
                        for echan in echans:
                            ...
                            plot_time_bin = data[8]
                            plot_time_sat = data[9]
                    data_both = np.array([residuals, counts, fit_curve, cgb, magnetic, earth_ang_bin, sun_ang_bin, sources_fit_curve, plot_time_bin, plot_time_sat])
            ...
            plt.title('Combined fit on the ' + ordinal(int(str(day)[4:6])) + ' ' + date.strftime('%B')[0:3] + ' ' + str(year), fontsize=40)
            ...
            plt.title('Combined residuals on the ' + ordinal(int(str(day)[4:6])) + ' ' + date.strftime('%B')[0:3] + ' ' + str(year), fontsize=40)
...
class writefile(object):
    ...
    def fits_data(self, day, detector_name, echan, data_type, residuals, counts, fit_curve, cgb, magnetic, earth_ang_bin, sun_ang_bin, sources_fit_curve, plot_time_bin, plot_time_sat, coeff_cgb, coeff_magn, coeff_earth, coeff_sun, sources_ang_bin, sources_names, sources_coeff, saa_coeffs):
    ...
    def mcilwain_fits(self, week, day):
        ...
        if x[0] == 0:
            ...
            lat_data2 = readfile.lat_spacecraft(readfile(), week2)#pay attention to the first and last day of the weekly file, as they are split in two!
