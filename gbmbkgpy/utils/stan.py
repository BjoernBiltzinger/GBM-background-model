...
class StanDataConstructor(object):
    ...
    def global_sources(self):
        ...
        global_counts = np.zeros((len(s), self._ntimebins,
    ...
    def continuum_sources(self):
        ...
        continuum_counts = np.zeros((num_cont_sources, self._ntimebins,
                                     self._ndets, self._nechans))
    ...
    def free_spectrum_sources(self):
        ...
        self._Ebins_in = np.vstack((self._response.responses[self._dets[0]].Ebin_in_edge[:-1],
        ...
        for k in s.keys():
            ...
            ar = np.zeros((self._ndets, len(self._geometry.geometry_times), self._num_Ebins_in, self._nechans))
        ...
        if base_response_array_earth is not None:
            ...
            eff_rsp_new_earth = interp1d(self._geometry.geometry_times,
                                         base_response_array_earth, axis=1)

            rsp_all_earth = np.swapaxes(np.array(np.swapaxes(eff_rsp_new_earth(self._time_bin_edges),-1,-2), dtype=float),0,1)

            # Trapz integrate over time bins
            base_response_array_earth = 0.5*(self._time_bins[:,1,np.newaxis,np.newaxis,np.newaxis]-
                                             self._time_bins[:,0,np.newaxis,np.newaxis,np.newaxis])*(rsp_all_earth[:,:-1]+rsp_all_earth[:,1:])

            self._base_response_array_earth = base_response_array_earth[2:-2].reshape(-1, self._num_Ebins_in)

        if base_response_array_cgb is not None:

            eff_rsp_new_cgb = interp1d(self._geometry.geometry_times,
                                       base_response_array_cgb, axis=1)

            rsp_all_cgb = np.swapaxes(np.array(np.swapaxes(eff_rsp_new_cgb(self._time_bin_edges),-1,-2), dtype=float),0,1)

            # Trapz integrate over time bins
            base_response_array_cgb = 0.5*(self._time_bins[:,1,np.newaxis,np.newaxis,np.newaxis]-
                                           self._time_bins[:,0,np.newaxis,np.newaxis,np.newaxis])*(rsp_all_cgb[:-1]+rsp_all_cgb[1:])

            self._base_response_array_cgb = base_response_array_cgb[2:-2].reshape(-1, self._num_Ebins_in)

        if base_rsp_ps_free is not None:

            eff_rsp_new_free_ps = interp1d(self._geometry.geometry_times,
                                           base_rsp_ps_free, axis=2)

            rsp_all_ps = np.swapaxes(np.array(np.swapaxes(eff_rsp_new_free_ps(self._time_bin_edges),-1,-2), dtype=float),1,2)

            # Trapz integrate over time bins
            base_rsp_ps_free = 0.5*(self._time_bins[:, 1, np.newaxis, np.newaxis, np.newaxis]-
                                     self._time_bins[:, 0, np.newaxis, np.newaxis, np.newaxis])*(rsp_all_ps[:, :-1]+rsp_all_ps[:, 1:])

            self._base_response_array_ps = base_rsp_ps_free[:,2:-2].reshape(base_rsp_ps_free.shape[0], -1, self._num_Ebins_in)
    ...
    def saa_sources(self):
        ...
        for i, s in enumerate(list(self._model.saa_sources.values())[:self._num_saa_exits]):
