...
class ContinuousData(object):
    ...
    def calc_earth_occ(self, angle):
        ...
        f = (np.cos(theta) - np.cos(opening_ang) * np.sin(phi)) / (np.cos(
        ...
        a = np.sqrt(
            1 - f ** 2 - (np.cos(opening_ang)) ** 2)  # diagonal of the overlapping crosssections of the spherical caps
        gamma_max = np.arctan(a / (
        2 * f))  # angle of the intersectionpoints on the plane perpendicular to the detector-orientation (top-view)
        ...
        f_e = (np.cos(opening_ang) - np.cos(theta) * np.sin(phi)) / (np.cos(
            phi))  # distance perpendicular to the detector-orientation to the intersection-plane of the spherical caps
        ...
        a_e = np.sqrt(
            1 - f_e ** 2 - (np.cos(theta)) ** 2)  # diagonal of the overlapping crosssections of the spherical caps
        gamma_max_e = np.arctan(a_e / (
        2 * f_e))  # angle of the intersectionpoints on the plane perpendicular to the detector-orientation (top-view)

        # calculate one part of the overlapping area of the spherical caps. This area belongs to the detector-cone
        A_d_an2 = 2 * (np.arctan2((np.sqrt(
            -(np.tan(beta)) ** 2 / ((np.sin(opening_ang)) ** 2) + (np.tan(beta)) ** 2 + 1) * np.sin(opening_ang)),
                                  np.tan(beta)) - np.cos(opening_ang) * np.arccos(
            np.tan(beta) / np.tan(opening_ang)) - (np.arctan2(
            (np.sqrt(-(np.tan(beta)) ** 2 / ((np.sin(beta)) ** 2) + (np.tan(beta)) ** 2 + 1) * np.sin(beta)),
            np.tan(beta)) - np.cos(beta) * np.arccos(np.tan(beta) / np.tan(beta))))

        # calculate the other part of the overlapping area. This area belongs to the earth-cone
        A_e_an2 = 2 * (np.arctan2(
            (np.sqrt(-(np.tan(beta_e)) ** 2 / ((np.sin(theta)) ** 2) + (np.tan(beta_e)) ** 2 + 1) * np.sin(theta)),
            np.tan(beta_e)) - np.cos(theta) * np.arccos(np.tan(beta_e) / np.tan(theta)) - (np.arctan2(
            (np.sqrt(-(np.tan(beta_e)) ** 2 / ((np.sin(beta_e)) ** 2) + (np.tan(beta_e)) ** 2 + 1) * np.sin(beta_e)),
            np.tan(beta_e)) - np.cos(beta_e) * np.arccos(np.tan(beta_e) / np.tan(beta_e))))
        ...
        # Apparently the numeric calculation of the analytic solution doesn't always return a value (probably because of runtime error). As a result there are several 'nan' entries in the A_an2 array. To get rid of those we interpolate over all the calculated solutions. We have chosen enough steps for the opening_ang to eliminate any errors due to this interpolation, because we get enough good results from the calculation.
        tck = interpolate.splrep(opening_ang[np.logical_not(np.isnan(A_an2))], A_an2[np.logical_not(np.isnan(A_an2))],
                                 s=0)
        A_an2 = interpolate.splev(opening_ang, tck, der=0)
