...
class SelectPointsources(object):
    ...
    def __init__(self, limit1550Crab, time_string=None, mjd=None):
        ...
        """
        assert (time_string is None) or (mjd is None), "Either enter time_string or mjd, not both!"

        if time_string is not None:
            t = Time(f"20{time_string[:2]}-{time_string[2:4]}-{time_string[4:6]}T00:00:00.000",
                     format='isot',
                     scale='utc')
        ...
        if not os.path.exists(get_path_of_data_file("background_point_sources/", "pointsources_swift.h5")):
            ...
            if not os.path.exists(get_path_of_data_file("background_point_sources/", "pointsources_swift.h5")):
                raise AssertionError("The pointsource_swift.h5 file still does not exist in the data folder. Aborting...")
    ...
    def ps_sign_swift(self):
        ...
        """
        with h5py.File(get_path_of_data_file("background_point_sources/", "pointsources_swift.h5"),"r") as h:
        ...
        names_s, rates_all_s, errors_all_s, ra_s, dec_s = names[sign_indices], rates_all[sign_indices], errors_all[sign_indices], ra[sign_indices], dec[sign_indices]
        ps_dict = {}
        for i in range(len(names_s)):
            ps_dict[names_s[i]] = {"Rates": rates_all_s[i], "Errors": errors_all_s[i], "Ra":ra_s[i], "Dec":dec_s[i]}
    ...
    def plot_ps(self):
        ...
        for i, key in enumerate(self._ps_dict.keys()):
            ...
            ax.scatter(np.where(np.deg2rad(ra)>np.pi,
                                np.deg2rad(ra)-2*np.pi,
                                np.deg2rad(ra)),
                       np.deg2rad(dec),
                       label=key,
                       color=colors[i],
    ...
    def animate_swift_ps(self, limit1550Crab=None, mjd_list=None, dpi=150):
        ...
        if mjd_list is None:
            mjd_list = np.linspace(self._time-50,
                                   self._time+50,
                                   100,
                                   dtype=int)
        ...
        ra_rad = np.where(np.deg2rad(ra) > np.pi,
                          np.deg2rad(ra)-2*np.pi,
                          np.deg2rad(ra))
        ...
        ani = FuncAnimation(fig, update, frames=np.arange(len(mjd_list)),
