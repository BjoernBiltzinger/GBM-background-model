...
class NNBackModel(object):
    ...
    def load_weights(self, file_name='trained_nn_model.h5'):
        fname = join(self.file_dir, file_name)
        print("Loading model weights '{0:}'".format(basename(fname)))
        self.model.load_weights(fname)

    def load_model(self, file_name='trained_nn_model.h5'):
        fname = join(self.file_dir, file_name)
        print("Loading model '{0:}'".format(basename(fname)))
        self.model = keras.models.load_model(fname)
        return

    def set_architecture(self, data, nfeatures, output_dim=1, learning_rate=8.0E-4, **kwargs):
        self.model = keras.Sequential()
        # The Input Layer :
        self.model.add(keras.layers.Dense(128, kernel_initializer='normal', input_dim=nfeatures, activation='relu'))

        # The Hidden Layers :
        self.model.add(keras.layers.Dense(256, kernel_initializer='normal', activation='relu'))
        self.model.add(keras.layers.Dense(256, kernel_initializer='normal', activation='relu'))
        self.model.add(keras.layers.Dense(256, kernel_initializer='normal', activation='relu'))

        # The Output Layer :
        self.model.add(keras.layers.Dense(output_dim, kernel_initializer='normal', activation='linear'))

        self.model.compile(optimizer=keras.optimizers.Adam(lr=learning_rate, epsilon=1.0E-8),
    ...
    def model_count_rates(self):
        if self._model_count_rates is None:
            self._model_count_rates = self.model.predict(self.features)
    ...
    def model_counts(self):
        if self._model_counts is None:
            self._model_counts = self.model.predict(self.features)
            for i in range(self._model_counts.shape[1]):
                self._model_counts[:, i] = self._model_counts[:, i] * self._total_time_bin_widths
    ...
    def display_model(self, echan, data_color='k', model_color='r', show_data=True, show_residuals=True,
        ...
        y = self.model_count_rates[:, echan]
