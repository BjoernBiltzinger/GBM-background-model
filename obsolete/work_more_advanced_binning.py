...
for i in range(0, len(bin_time_mid)):
    bin_time_slot = np.sum(bin_time_mid[i:i+binning-1])/binning
    bin_time_temp.append(bin_time_slot)
    bin_count_slot = np.sum(counts[i:i+binning-1])/binning
    bin_count_temp.append(bin_count_slot)
    i = i + binning

bin_time_mid = np.array(bin_time_temp)
counts = np.array(bin_count_temp)    
